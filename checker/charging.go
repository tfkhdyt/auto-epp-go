...
func CheckChargingStatus() bool {
	...
	for _, file := range files {
		...
		if file.IsDir() || (fileInfo.Mode()&os.ModeSymlink != 0) {
			typeData, err := os.
				ReadFile(filepath.Join(powerSupplyPath, file.Name(), "type"))
			if err != nil {
				continue
			}
			...
			if supplyType == "Mains" {
				onlineData, err := os.
					ReadFile(filepath.Join(powerSupplyPath, file.Name(), "online"))
				if err != nil {
					continue
				}
				...
				statusData, err := os.
					ReadFile(filepath.Join(powerSupplyPath, file.Name(), "status"))
				if err != nil {
					continue
				}
